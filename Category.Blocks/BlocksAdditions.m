...
@implementation DeallocBlockValue 

@synthesize value, deallocBlock;

+ (DeallocBlockValue *) valueWithPointer:(const void *) aPointer deallocBlock:(DeallocBlock) block
{
    DeallocBlockValue *val = [[DeallocBlockValue alloc] init];
    val.value = [NSValue valueWithPointer:aPointer];
    val.deallocBlock = block;
    return val;
}
...
- (void) deallocBlock:(DeallocBlock) block
{
    if (self.deallocBlock == block)
	{
        return;
    }
    deallocBlock = [block copy];
}
